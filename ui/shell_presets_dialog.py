...
class ShellPresetsDialog(QDialog):
    ...
    def __init__(self, current_presets: list[ShellPreset], parent: QWidget | None = None) -> None:
        ...
        for preset in current_presets:
            self._append_row(preset.label, preset.host, preset.raw_command)
    ...
    def _append_row(self, label: str, host: str, command: str) -> None:
        row = self._table.rowCount()
        self._insert_row(row, label, host, command)

    def _insert_row(self, row: int, label: str, host: str, command: str) -> None:
    ...
    def _move_row(self, delta: int) -> None:
        ...
        self._write_row(cur, *target_data)
        self._write_row(target, *cur_data)
        self._table.selectRow(target)
    ...
    def _write_row(self, row: int, label: str, host: str, command: str) -> None:
    ...
    def _refresh_buttons(self) -> None:
        ...
        self._btn_remove.setEnabled(cur >= 0 and rows > 1)
    ...
    def _collect_presets(self) -> list[ShellPreset] | None:
        ...
        for row in range(self._table.rowCount()):
            ...
            result.append(ShellPreset(label=label, host=host, raw_command=command))
        if not result:
            self._warn("至少需要一条预设")
            return None
        return result
